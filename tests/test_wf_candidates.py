...
def test_cooldown_hours_is_used():
    """cooldown_hours param should be converted to bars."""
    params = {"cooldown_hours": 48}
    engine = BacktestEngine([], strategy_params=params)
    assert engine._cooldown_bars == 48 * 12  # 12 x 5m bars per hour
...
def test_defaults_when_no_params():
    ...
    assert engine._cooldown_bars == 72 * 12  # default 72h cooldown
