...
async def get_equity_history(days: int = Query(7, ge=1, le=90)):
    """Timestamped equity snapshots for time-series charting."""
    async with get_session() as session:
        snaps = await get_snapshots(session, days=days)
    return [
        {
            "timestamp": s.snapshot_at.isoformat() if s.snapshot_at else None,
            "equity": s.total_equity_eur,
        }
        for s in snaps
        if s.snapshot_at is not None
    ]
